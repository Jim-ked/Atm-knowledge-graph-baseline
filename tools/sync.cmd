...
if exist "%PROJECT_ROOT%\target\atm-knowledge-graph-1.0-SNAPSHOT.jar" if exist "%PROJECT_ROOT%\target\lib\*.jar" goto runtime_sync

if not exist "%MAVEN_COMMAND%" where %MAVEN_COMMAND% >nul 2>nul
if errorlevel 1 goto missing_maven
pushd "%PROJECT_ROOT%"
call "%MAVEN_COMMAND%" -q -DskipTests compile org.codehaus.mojo:exec-maven-plugin:3.5.0:java "-Dexec.mainClass=org.atmkg.tools.SyncControlMain"
...
:runtime_sync
pushd "%PROJECT_ROOT%"
call "%JAVA_COMMAND%" -cp "target\atm-knowledge-graph-1.0-SNAPSHOT.jar;target\lib\*" org.atmkg.tools.SyncControlMain "%PROJECT_ROOT%"
set "RESULT=%ERRORLEVEL%"
popd
exit /b %RESULT%
